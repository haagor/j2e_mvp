...
public class BankAPI {
	...
	public boolean performPayment(String creditCard, double value) {
		Integer id = pay(buildRequest(creditCard, value));
		return isValid(getPaymentStatus(id));
	}

	private JSONObject buildRequest(String creditCard, double value) {
		// Building payment request
		return new JSONObject().put("CreditCard", creditCard).put("Amount", value);
	}
